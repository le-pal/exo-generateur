...
import { getStudents, getReferenceData, createSession } from '../api/client.ts';
...
export default function GeneratePage() {
  ...
  const mutation = useMutation<SessionView, AxiosError<{ error: string }>, GenerateForm>({
    ...
    onSuccess: (session) => void navigate(`/session/${session.id}`),
  ...
  const set = (key: keyof GenerateForm) => (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>) =>
    setForm(prev => ({ ...prev, [key]: key === 'num_exercises' ? Number(e.target.value) : e.target.value }));
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div className="card">
          ...
          <div className="flex gap-3 items-end">
            ...
            {selectedStudent && (
              <button type="button" onClick={() => void navigate(`/history/${selectedStudent.id}`)} className="btn-secondary text-xs">
                Voir l'historique
              </button>
            )}
          </div>
          {students.length === 0 && (
            <p className="text-sm text-amber-600 mt-2">
              ...
              <button type="button" onClick={() => void navigate('/admin')} className="underline">créer un élève dans Admin</button>
            </p>
          )}
        </div>
        ...
        <div className="card">
          ...
          <div>
            ...
            <input type="number" className="input w-28" min={1} max={20} value={form.num_exercises} onChange={set('num_exercises')} />
          </div>
        </div>
        ...
        <div className="card">
          ...
          <button type="button" onClick={() => fileRef.current?.click()} className="btn-secondary w-full border-dashed">
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              {files.map((f, i) => (
                <div key={i} className="flex items-center gap-1 bg-blue-50 text-blue-700 rounded-lg px-2 py-1 text-xs">
                  ...
                  <button type="button" onClick={() => setFiles(prev => prev.filter((_, idx) => idx !== i))} className="hover:text-red-500 ml-1">✕</button>
                </div>
              ))}
            </div>
          )}
        ...
        <button type="submit" className="btn-primary w-full py-3 text-base" disabled={mutation.isPending}>
          {mutation.isPending ? <Spinner label="Génération en cours…" /> : 'Générer les exercices →'}
        </button>
      </form>
    </div>
  );
