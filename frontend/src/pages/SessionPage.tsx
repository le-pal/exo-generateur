...
export default function SessionPage() {
  ...
  const { data: session, isLoading, isError } = useQuery<SessionView>({
    queryKey: ['session', id],
    queryFn: () => getSession(Number(id)),
    refetchOnWindowFocus: false,
  });

  const handleAnswer = useCallback((exerciseId: number, value: string) => {
    clearTimeout(saveTimers.current[exerciseId]);
    saveTimers.current[exerciseId] = setTimeout(() => {
      void saveAnswer(Number(id), exerciseId, value);
    }, 800);
  }, [id]);

  const correctMutation = useMutation<SessionView, AxiosError<{ error: string }>>({
    mutationFn: () => correctSession(Number(id)),
    onSuccess: (data) => {
    ...
    <div className="max-w-2xl mx-auto">
      <div className="mb-6">
        ...
        {isCorrected && (
          <div className="mt-4 p-4 bg-blue-50 rounded-xl flex items-center justify-between">
            <div>
              <p className="font-semibold text-blue-900">{correct.length}/{exercises.length} bonnes réponses</p>
              <p className="text-sm text-blue-700 mt-0.5">
                {earnedPoints >= totalPoints * 0.8 ? 'Excellent travail !'
                  : earnedPoints >= totalPoints * 0.5 ? 'Bon travail, continue !'
      ...
      {!isCorrected && (
        <div className="mt-8 space-y-3">
          {correctionError && (
            <div className="bg-red-50 border border-red-200 text-red-700 rounded-lg px-4 py-3 text-sm">{correctionError}</div>
          )}
          <button onClick={handleCorrect} disabled={correcting} className="btn-primary w-full py-3 text-base">
            {correcting
              ? <Spinner label="Correction en cours…" />
      ...
      {isCorrected && (
        <div className="mt-8 flex gap-3 justify-center">
          <button onClick={() => void navigate('/')} className="btn-primary">Nouvel exercice</button>
          <button onClick={() => void navigate(`/history/${session.student_id}`)} className="btn-secondary">Voir l'historique</button>
        </div>
      )}
    </div>
  );
  ...
  return (
    <svg viewBox="0 0 60 60" className="w-full h-full -rotate-90">
      ...
      <text x="30" y="35" textAnchor="middle"
        style={{ fontSize: 12, transform: 'rotate(90deg)', transformOrigin: '30px 30px', fill: '#1d4ed8', fontWeight: 'bold' }}>
        {Math.round(pct * 100)}%
      </text>
    </svg>
  );
